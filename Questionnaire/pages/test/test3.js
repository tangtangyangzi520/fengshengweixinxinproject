...
Page({
  ...
  onFinishQuestion() {
    ...
    wx.request({
      ...
      success: res => {
        console.log(res)
        wx.setStorageSync("resultData3", res.data.data);
  ...
  onRedirectTo(timeout) {
    ...
    setTimeout(
      function () {
        wx.redirectTo({
          url: '/pages/result/result3?title=' + title + '&topicId=' + topicId,
        })
      }, timeout)
  },
  ...
  onShareAppMessage: function () {
    ...
    return {
      title: title,
      path: "/pages/preTest/preTest3?topicId=" + topicId + "&title=" + title,
      success: res => {
        console.log(res)
      }
    }
